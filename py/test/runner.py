...
class FastlineVpnTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FastlineVpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FastlineVpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FastlineVpnTestRunner.getenv("FASTLINE_VPN_TEST_LIVE")
        override = FastlineVpnTestRunner.getenv("FASTLINE_VPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FastlineVpnTestRunner.getenv(key)
        ...
        explain = FastlineVpnTestRunner.getenv("FASTLINE_VPN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FASTLINE_VPN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FastlineVpnTestRunner._test_control is not None:
            return FastlineVpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FastlineVpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FastlineVpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FastlineVpnTestRunner.load_test_control()
...
def load_env_local():
    FastlineVpnTestRunner.load_env_local()


def env_override(m):
    return FastlineVpnTestRunner.env_override(m)


def entity_data(v):
    return FastlineVpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FastlineVpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FastlineVpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FastlineVpnTestRunner.load_test_control()


def live_client_options():
    return FastlineVpnTestRunner.live_client_options()


def live_delay_ms():
    return FastlineVpnTestRunner.live_delay_ms()
