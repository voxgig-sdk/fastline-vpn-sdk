...
class TestServerEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Server(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _server_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "server." + _op, "live" if _live else "unit")
            if _skip:
        ...
        # CREATE
        server_ref01_ent = client.Server(None)
...
def _server_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/server/ServerTestData.json")
