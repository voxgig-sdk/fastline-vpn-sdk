...
boru-driven command-line client **and** interactive REPL for the FastlineVpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fastline-vpn-cli)
make build

# 2. See usage (words, entities, env vars)
./fastline-vpn-cli --help
...
FASTLINE_VPN_BASE=https://api.example.com ./fastline-vpn-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fastline-vpn-cli
   ```
...
Run with no arguments to open a REPL (prompt `fastline-vpn>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `server`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fastline-vpn-cli`. |
...
(`./fastline-vpn-cli <expr>`) and interactively (the REPL), and expressions compose the
