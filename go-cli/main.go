// Command FastlineVpn-cli is a CLI + REPL for the FastlineVpn SDK,
...
const prompt = "fastline-vpn"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "server"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FASTLINE_VPN_APIKEY (api key), FASTLINE_VPN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FASTLINE_VPN_APIKEY carries the API key and
	// FASTLINE_VPN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FASTLINE_VPN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FASTLINE_VPN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFastlineVpnSDK(opts)
