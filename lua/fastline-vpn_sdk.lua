...
local FastlineVpnSDK = {}
FastlineVpnSDK.__index = FastlineVpnSDK
...
FastlineVpnSDK._make_feature = _make_feature


function FastlineVpnSDK.new(options)
  local self = setmetatable({}, FastlineVpnSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FastlineVpnSDK:options_map()
...
function FastlineVpnSDK:get_utility()
  return Utility.copy(self._utility)
end


function FastlineVpnSDK:get_root_ctx()
  return self._rootctx
end


function FastlineVpnSDK:prepare(fetchargs)
...
function FastlineVpnSDK:direct(fetchargs)
...
-- by passing it.
function FastlineVpnSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FastlineVpnSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FastlineVpnSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:Server():list() / client:Server():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function FastlineVpnSDK:Server(data)
  local EntityMod = require("entity.server_entity")
  if data == nil then
    if self._server == nil then
      self._server = EntityMod.new(self, nil)
    end
    return self._server
  end
...
function FastlineVpnSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FastlineVpnSDK.new(sdkopts)
  sdk.mode = "test"
