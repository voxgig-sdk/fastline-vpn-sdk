...
function ServerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ServerEntity)
  self._name = "server"
  self._client = client
...
function ServerEntity:get_name()
  return self._name
end


function ServerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ServerEntity.new(self._client, opts)
end
...
function ServerEntity:mark_deleted()
  self._deleted = true
end


function ServerEntity:deleted()
  return true == self._deleted
end


function ServerEntity:data_set(args)
  if args ~= nil then
...
function ServerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ServerEntity:match_set(args)
  if args ~= nil then
...
function ServerEntity:match_get()
...
function ServerEntity:stream(action, args, callopts)
...
---@param reqdata ServerCreateData
---@param ctrl? table
---@return Server
---@return string? err
function ServerEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ServerEntity:_run_op(ctx, post_done)
  local utility = self._utility
