...
describe("ServerEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set FASTLINEVPN_TEST_SERVER_ENTID JSON to run live")
      return
    end
...
function server_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("FASTLINEVPN_TEST_SERVER_ENTID")
  ...
  if env["FASTLINEVPN_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["FASTLINEVPN_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
