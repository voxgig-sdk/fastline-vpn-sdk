...
from projectname_sdk import FastlineVpnSDK
from projectname_sdk.core.spec import FastlineVpnSpec
from projectname_sdk.core.result import FastlineVpnResult
from projectname_sdk.core.response import FastlineVpnResponse
from projectname_sdk.core.operation import FastlineVpnOperation
from projectname_sdk.core.error import FastlineVpnError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import FastlineVpnBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return FastlineVpnError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = FastlineVpnSDK.test(None, None)
    ...
    def test_feature_init_basic(self):
        init_client = FastlineVpnSDK.test(None, None)
        ...
        class TestInitFeature(FastlineVpnBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(FastlineVpnBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = FastlineVpnSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = FastlineVpnSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = FastlineVpnOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = FastlineVpnSDK.test(None, setup_opts)
