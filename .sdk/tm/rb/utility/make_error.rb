...
module FastlineVpnUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FastlineVpnContext.new({}, nil)
    end
    op = ctx.op || FastlineVpnOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FastlineVpnResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FastlineVpnError) ? err.msg : err.to_s
    ...
    sdk_err = FastlineVpnError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FastlineVpnError)
