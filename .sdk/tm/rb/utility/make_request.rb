...
module FastlineVpnUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FastlineVpnResponse.new({})
    ...
    if fetch_err
      ...
      response = FastlineVpnResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FastlineVpnResponse.new(fetched)
    else
