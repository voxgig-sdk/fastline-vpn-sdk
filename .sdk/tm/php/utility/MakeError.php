...
class FastlineVpnMakeError
{
    public static function call(?FastlineVpnContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FastlineVpnContext([], null);
        }
        $op = $ctx->op ?? new FastlineVpnOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FastlineVpnResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FastlineVpnError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FastlineVpnError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FastlineVpnError) {
