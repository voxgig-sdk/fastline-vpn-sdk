...
{
    private static function utility(): FastlineVpnUtility
    {
        return new FastlineVpnUtility();
    }
    ...
    private static function ctx(array $over = []): FastlineVpnContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FastlineVpnContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FastlineVpnOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FastlineVpnError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FastlineVpnOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FastlineVpnMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FastlineVpnOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FastlineVpnMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FastlineVpnOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FastlineVpnMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FastlineVpnSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FastlineVpnMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FastlineVpnResponse([]),
            'result' => new FastlineVpnResult([]),
        ]);
        [, $err] = FastlineVpnMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FastlineVpnSpec([]),
            'response' => null,
            'result' => new FastlineVpnResult([]),
        ]);
        [, $err] = FastlineVpnMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FastlineVpnSpec(['step' => 's']),
            'response' => new FastlineVpnResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FastlineVpnResult([]),
        ]);
        [, $err] = FastlineVpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FastlineVpnSpec(['step' => 's']),
            'response' => new FastlineVpnResponse(self::resp(200, ['v' => 1])),
            'result' => new FastlineVpnResult([]),
        ]);
        [, $err] = FastlineVpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FastlineVpnSpec(['step' => 's']),
            'response' => new FastlineVpnResponse(self::resp(200, ['v' => 2])),
            'result' => new FastlineVpnResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FastlineVpnMakeResponse::call($ctx);
    ...
    {
        $preset = new FastlineVpnResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FastlineVpnSpec([]),
            'response' => new FastlineVpnResponse([]),
            'result' => new FastlineVpnResult([]),
        ...
        [$got, $err] = FastlineVpnMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FastlineVpnResult([])]);
        [, $err] = FastlineVpnMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FastlineVpnSpec([]), 'result' => null]);
        [, $err] = FastlineVpnMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FastlineVpnSpec(['step' => 's']),
            'result' => new FastlineVpnResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FastlineVpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FastlineVpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FastlineVpnSpec(['step' => 's']),
            'result' => new FastlineVpnResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FastlineVpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FastlineVpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FastlineVpnResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FastlineVpnSpec([]),
            'result' => new FastlineVpnResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FastlineVpnMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FastlineVpnMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FastlineVpnError('boom', 'boom');
        $utility->fetcher = function (FastlineVpnContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FastlineVpnSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FastlineVpnMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FastlineVpnContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FastlineVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FastlineVpnResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FastlineVpnMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FastlineVpnContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FastlineVpnSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FastlineVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FastlineVpnResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FastlineVpnSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FastlineVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FastlineVpnMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FastlineVpnMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FastlineVpnResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FastlineVpnDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FastlineVpnResult(['ok' => false])]);
        $this->expectException(FastlineVpnError::class);
        FastlineVpnDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FastlineVpnResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FastlineVpnMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FastlineVpnMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FastlineVpnBaseFeature();
        $b = new FastlineVpnBaseFeature();
        FastlineVpnFeatureAdd::call($ctx, $a);
        FastlineVpnFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FastlineVpnBaseFeature
    {
        $f = new FastlineVpnBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FastlineVpnFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FastlineVpnContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FastlineVpnSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FastlineVpnPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FastlineVpnPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FastlineVpnResponse(['headers' => null]),
            'result' => new FastlineVpnResult([]),
        ]);
        FastlineVpnResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        FastlineVpnResultBody::call($ctx);
