...
class FastlineVpnSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FastlineVpnHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FastlineVpnHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FastlineVpnFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FastlineVpnFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FastlineVpnUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FastlineVpnHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FastlineVpnHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FastlineVpnHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FastlineVpnError($op . "_allow",
                "FastlineVpnSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FastlineVpnHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FastlineVpnError("graphql_error",
                "FastlineVpnSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Server()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->server()
    // resolves here too.
    public function Server($data = null)
    {
        require_once __DIR__ . '/entity/server_entity.php';
        if ($data === null) {
            if ($this->_server === null) {
                $this->_server = new ServerEntity($this, null);
            }
            return $this->_server;
        }
        return new ServerEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new FastlineVpnSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
