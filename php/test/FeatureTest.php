...
class FtCtrl extends FastlineVpnControl
{
...
class FtClient
{
    ...
    private FastlineVpnUtility $utility;

    public function __construct(FastlineVpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): FastlineVpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FastlineVpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FastlineVpnContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FastlineVpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FastlineVpnConfig::make_feature($name);
    ...
    private static function buildUrl(FastlineVpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FastlineVpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FastlineVpnResponse($fetched);
        }
    ...
    private function fail(FastlineVpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FastlineVpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
