...
class ServerEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ServerEntity($this->_client, $opts);
    }

    /**
     * @param Server|array $args Server data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FastlineVpnHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Server|array The current Server data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Server fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FastlineVpnHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Server fields).
     */
     ...
     * Create a new Server.
     *
     * @param ServerCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ServerCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Server|array The created Server as an assoc-array at the
     *   SDK boundary; throws FastlineVpnError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = FastlineVpnHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
