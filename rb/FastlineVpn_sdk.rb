...
require_relative 'FastlineVpn_types'
...
class FastlineVpnSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FastlineVpnHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FastlineVpnHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FastlineVpnFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FastlineVpnUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FastlineVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FastlineVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FastlineVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FastlineVpnHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FastlineVpnError.new(
        "graphql_error", "FastlineVpnSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Server.list / client.Server.load({ "id" => ... })
  def Server(data = nil)
    require_relative 'entity/server_entity'
    ServerEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FastlineVpnSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
