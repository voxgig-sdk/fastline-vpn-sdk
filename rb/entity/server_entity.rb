...
class ServerEntity
  ...
  def make
    opts = @_entopts.dup
    ServerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FastlineVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Server, Hash] the current Server data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FastlineVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Server fields)
  def match_get
  ...
  # Create a new Server.
  #
  # @param reqdata [ServerCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Server, Hash] the created Server; raises FastlineVpnError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = FastlineVpnHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
