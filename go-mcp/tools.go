...
type Args struct {
	Entity string         `json:"entity" jsonschema:"server"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.FastlineVpnSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "fastline-vpn_list",
		Description: "List records from FastlineVpn. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "fastline-vpn_load",
		Description: "Load a single record from FastlineVpn. " +
...
func runOp(client *sdk.FastlineVpnSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.FastlineVpnSDK, name string) (sdk.FastlineVpnEntity, error) {
	switch strings.ToLower(name) {
	case "server":
		return client.Server(nil), nil
