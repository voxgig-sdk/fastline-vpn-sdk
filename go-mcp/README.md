...
[MCP](https://modelcontextprotocol.io) server exposing the FastlineVpn SDK as
two agent tools — `fastline-vpn_list` and `fastline-vpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fastline-vpn-mcp)
make build

# 2. Provide credentials via the environment
export FASTLINE_VPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fastline-vpn \
  -- /absolute/path/to/fastline-vpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fastline-vpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fastline-vpn_list: first page of records
{ "entity": "server" }
{ "entity": "server", "query": { } }

// fastline-vpn_load: one record by id
{ "entity": "server", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fastline-vpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fastline-vpn \
     -- "$PWD"/dist/*/fastline-vpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fastline-vpn_list` and `fastline-vpn_load` tools now appear
   in new sessions. Ask the agent to *"list server using fastline-vpn"*
   and it calls `fastline-vpn_list` with `{"entity":"server"}`.
...
```sh
export FASTLINE_VPN_APIKEY=sk_live_xxx            # API key
export FASTLINE_VPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `fastline-vpn_list` tool
...
### Call the `fastline-vpn_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `fastline-vpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fastline-vpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FASTLINE_VPN_APIKEY` | API key sent with every request. |
| `FASTLINE_VPN_BASE` | Optional override of the API base URL. |
...
```sh
./fastline-vpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fastline-vpn_load","arguments":{"entity":"server","query":{"id":1}}}}'
```
