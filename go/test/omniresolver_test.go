...
import (
	...
	omni "github.com/voxgig-sdk/fastline-vpn-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FastlineVpnError); is {
				out := map[string]any{
					"name":    "FastlineVpnError",
...
func omniCtx(arg any, client *sdk.FastlineVpnSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FastlineVpnSDK); is {
			client = live
