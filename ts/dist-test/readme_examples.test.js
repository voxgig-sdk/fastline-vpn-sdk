...
const TEST_SEED = { "entity": { "server": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+FastlineVpnSDK\b/.test(code) ||
        /\bFastlineVpnSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+FastlineVpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/FastlineVpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
