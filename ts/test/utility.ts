...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FASTLINEVPN_TEST_LIVE ||
    'TRUE' === process.env.FASTLINEVPN_TEST_OVERRIDE
  ) {
  ...
  m.FASTLINEVPN_TEST_EXPLAIN = process.env.FASTLINEVPN_TEST_EXPLAIN || m.FASTLINEVPN_TEST_EXPLAIN
