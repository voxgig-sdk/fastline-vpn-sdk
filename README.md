...
FastLine VPN API client, generated from the OpenAPI spec.

The FastLine VPN API provides users with a fast and secure way to access the internet.

Learn more about FastLine VPN API at [freevpn-rel.fastline.club](https://freevpn-rel.fastline.club).

This is an unofficial SDK for the FastLine VPN public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Server — that you
...
```bash
cd go-mcp && go build -o fastline-vpn-mcp .
```
...
```json
{
  "mcpServers": {
    "fastline-vpn": {
      "command": "/abs/path/to/fastline-vpn-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Server** | The Server entity (create). | `/ajax/servers` |
...
```go
import sdk "github.com/voxgig-sdk/fastline-vpn-sdk/go"
...
Generated from the FastLine VPN API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
